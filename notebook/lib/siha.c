...
void loadData(uint32_t* inbuff, int offset, int size) {
	std::memcpy(xrt_buff_ptr+offset, (uint32_t*)inbuff, size*4);
}

void retrieveData(uint32_t* outbuff, int offset, int size) {
	std::memcpy((uint32_t*)outbuff, xrt_buff_ptr+offset, size);
}